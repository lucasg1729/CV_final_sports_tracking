...
detection (occluded, left frame, etc.).
...
def iou(box_a: np.ndarray, box_b: np.ndarray) -> float:
    ...
    (zero or negative area). The latter is a defensive guard -- a buggy
...
def iou_matrix(tracks: np.ndarray, detections: np.ndarray) -> np.ndarray:
    ...
    Vectorized for speed -- this gets called once per frame per clip,
    and a naive double loop would dominate runtime when there are many
    objects on screen.
...
def associate(
    ...
    # (Equivalent to 1 - IOU since adding a constant doesn't change the
    # optimum, and -IOU avoids the +1 of clutter.)
    cost = -iou_mat
