...
Why the cache exists: YOLO inference is slow on CPU (a few seconds per
frame for v8x, sub-second per frame for v8n). A typical SportsMOT clip
is ~750 frames, so a single inference pass is several minutes to tens
of minutes. The tracker, by contrast, runs in milliseconds. We never
...
The cache also fixes the inputs to the tracker for reproducibility:
...
# people; we don't track balls/equipment in this project.
PERSON_CLASS_ID = 0
